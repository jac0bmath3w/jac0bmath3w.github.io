...
1. load config and connect
   ...
   - looks up matching profile (in this case it is dbt-intro) in ~/.dbt/profiles.yml
   - connects to data/warehouse.duckdb (as given in profiles.yml).
   - sets concurrency to 2 threads (as given in profiles.yml)
2. parse the project
   - scans 'models/' for .sql and schema.yml (In our case, schema = 'raw' is given in the model).
   - builds from the source mentioned in the model. (Later in staging, intermediate etc, we reference the models built in raw).
