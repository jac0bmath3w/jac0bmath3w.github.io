...
Only one model was tried as I had other committments.

### Feature engineering & alignment (the three helpers)

#### update_failure_data(...) — enrich the wheel-level table

Purpose. Add simple, interpretable covariates to each wheel-month row before telemetry joins: prior failure count, a loaded-mileage ratio, and an equipment-level mileage slope.

Inputs.

failure_data: wheel-level table with wheel_id, equipmentnumber, recordmonth, applieddate_used, failedin30days, partmileage, …

mileage_ratio_summary: per-equipment table with trimmed_mean of loaded_to_added ratio

totalmileage_slope_summary: per-equipment table with coefficient_mean from totalmileage segment slopes
...
Sort by wheel_id, applieddate_used, recordmonth; compute cum_failures = cumsum(failedin30days) then shift to form prior failures.

Left-join loaded_ratio and mileage_slope by equipmentnumber.

Derive loadedmileage = partmileage × loaded_ratio.

Why it matters. These three features capture history, usage mix, and fleet-level drift without touching the high-dimensional telemetry yet.
...
Why it matters. This yields a month-by-month panel per wheel that respects the install date and stops at the evaluation month—perfect for building calendar lags next.

#### generate_calendar_lags_with_applied_condition(...) — calendar-anchored lags, leakage-safe

Purpose. For each wheel-month row, attach up to k months of historical values (lag1…lagk) for selected telemetry features—but never from before the wheel’s install date.
...
Lags are calendar-true (one per month), not row-shifted; they respect missing months.

The install-date mask guarantees that early lags don’t peek before a wheel existed.

Including applied_col in the merge key keeps separate installs of the same wheel_id from bleeding into each other.

Where these sit in the modeling pipeline
...
Persist both the full lagged table and the reduced feature matrix for reproducibility.
...
Evaluation: Predict class probabilities on validation; report log loss. (Accuracy/confusion matrix can be added later if you like.)

The script saves the tuned model via joblib.dump(...). The filename currently encodes the log-loss (e.g., xgboost_multiclass_model_ll:0.846.pkl), which makes model selection easy during iteration.

Validity checks & edge cases (recommended to mention)

Install boundaries: Verify that for early months after install, some lags are NA by design; that’s expected and correct.

Multiple installs per wheel_id: Because merges include applieddate_used, features remain per-install, not just per wheel ID.

Calendar gaps: The lagging is calendar-based; if a month is missing in WILD, the corresponding lag for that month won’t be back-filled—consider simple imputation in a later iteration if needed.

Alignment: The final filter recordmonth_wild == recordmonth ensures features/label share the same month; if you ever switch to predicting future failure, remove this filter and re-define which month is the target.